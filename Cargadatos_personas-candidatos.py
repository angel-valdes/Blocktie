...
for index, row in datatable.iterrows():
    ...
    mydb.commit()
    print(id_persona, row.titulo_perfil, row.experiencia_total)
    x = str(row.titulo_perfil)
    if x == 'nan':
        x = del_nan(x)
    cur.execute('INSERT INTO candidato (id_persona, extra, experiencia_laboral) VALUES (%s, %s, %s)',
                (id_persona, x, row.experiencia_total))
    mydb.commit()
    ...
    for i in lista_exp_lab:
        ...
        if i[0] == '':
            id_empresa = [(1103,)]
        cur.execute('INSERT INTO experiencia_laboral (id_persona, id_empresa, cargo) VALUES (%s, %s, %s)',
                    (id_persona, id_empresa[0][0], i[1]))
        mydb.commit()
    # certificados
    ...
    for i in lista_certificados:
        ...
        if i[0] == '':
            id_empresa = [(19,)]
        cur.execute('INSERT INTO certificado (id_persona_candidato, id_institucion, titulo, menciones) VALUES (%s, %s, %s, %s)',
                    (id_persona, id_institucion[0][0], x, i[2]))
