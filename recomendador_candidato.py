...
id_candidato = "9" #parametro_funcion
...
# obtener oferta
mycursor.execute('SELECT empresa.nombre, oferta.cargo_requerido, empresa.industria, oferta.area_de_trabajo, oferta.experiencia_laboral_req FROM empresa, oferta, recomendaciones WHERE empresa.id = oferta.id_empresa AND oferta.id = recomendaciones.id_oferta AND recomendaciones.id_candidato = {} ORDER BY recomendaciones.similarity DESC LIMIT 0,5'.format(int(id_candidato)))
